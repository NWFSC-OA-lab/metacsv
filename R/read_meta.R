readMeta <- function(fileName){
  ...
  inFile <- read.csv(fileName, stringsAsFactors=FALSE, header = FALSE)
  ...
  if(inFile[1,1] == "NWFSC_OA_format_file"){
    ...
    dGeneralMeta <- read.csv(fileName, stringsAsFactors=FALSE,
    ...
    dVariableMeta <- read.csv(fileName, stringsAsFactors=FALSE,
    ...
    dData <- read.csv(fileName, stringsAsFactors=FALSE,
    ...
    metaFileName <- word(fileName, sep = ".csv")
